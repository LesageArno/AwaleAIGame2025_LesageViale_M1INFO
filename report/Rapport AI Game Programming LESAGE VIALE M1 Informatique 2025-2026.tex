...
%\section*{Introduction}
    ...
    Le but d'un joueur est de maximiser le nombre de graines qu'il a récolté, le joueur en ayant le plus à la fin de la partie étant le gagnant du jeu.
...
\subsection{Une première tentative orienté attaque \textit{raw}}\label{eval:raw}
...
Cela veut dire que l'évaluation des modèles peut se faire en un unique match singulier pour chaque pairs de fonction d'évaluation.
Pour déterminer la meilleur fonction d'évaluation, il nous suffire in-fine de compter le nombre de victoire et de prendre celle maximisant le nombre de victoire.
...
Premièrement, au delà des résultats qui nous intéressait initialement, nous observons un potentiel biais en faveur de \texttt{[J1]} dans cette version du jeu de l'Awalé. 
Nous emmétrons donc l'hypothèse suivante, à confirmer dans la suite :
\begin{hypothesis}[Biais favorable à \texttt{[J1]}]
    Il existe un biais dans la version modifiée de l'Awalé rendant la victoire de \texttt{[J1]} plus simple comparé à celle de \texttt{[J2]}.
\end{hypothesis}

En faisant abstraction de ce biais, nous observons tout de même deux phénomènes importants :
\begin{itemize}[noitemsep,topsep=0pt]
    \item La fonction d'évaluation \textbf{\textit{raw}} et \textbf{\textit{corrected}} sont équivalentes du point de vue de \texttt{[J1]} en termes de résultat, mais cette dernière se révèle bien plus efficace au mains de \texttt{[J2]} en réduisant le nombre de capture de \texttt{[J1]} de 33.1\% en moyenne et en augmentant les captures de \texttt{[J2]} de 260\% en moyenne par rapport à \textbf{\textit{raw}}.
    \item De même, la fonction d'évaluation \textbf{\textit{defence}} ne semble pas convenir à \texttt{[J1]} étant donné que celle-ci se retrouve affaiblis face à une stratégie défensive ou très offensive de l'adversaire. Néanmoins, celle-ci semble bien fonctionner dans les mains de \texttt{[J2]} à condition que \texttt{[J1]} suive aussi une stratégie défensive.
\end{itemize} 
...
Ici, nous observons bien le biais mentionnée précédemment d'une manière d'autant plus amplifié par la profondeur.
...
Concernant les résultats en eux même la fonction d'évaluation \textit{\textbf{corrected}} reste équivalente à \textbf{\textit{raw}} du point de vue de \texttt{[J1]}, mais se détache d'autant plus du point de vue de \texttt{[J2]} en maximisant ces captures par rapport à toutes les autres fonctions d'évaluations.
Cela confirme son rôle de fonction d'évaluation maîtresse pour maximiser les résultats offensifs.

D'un autre côté, la fonction d'évaluation défensive voit ses capacités augmenter en assurant au global une meilleur minimisation du score adverse contre des stratégies offensives légère ou défensive, mais reste vulnérable contre des fonctions très offensive comme \textit{\textbf{corrected}} d'un point de vue de \texttt{[J1]}.
Néanmoins, du point de vue de \texttt{[J2]}, tout change dans la mesure où l'ensemble des matchs où la stratégie défensive a été utilisé, le score de \texttt{[J1]} a été grandement minimisé par rapport aux autres fonctions d'évaluation, cela avançant ces capacités défensives dans le cas de \texttt{[J2]} uniquement.
...
\begin{itemize}[noitemsep,topsep=0pt]
    ...
    Cela est particulièrement vrai dès que la fonction d'évaluation \textbf{\textit{raw}} est impliqué. Cela laisse penser que les erreurs d'implémentations cités dans la sous-section~\ref{eval:raw} interagissent mal avec l'Alpha-Beta rendant les coupes moins probables.
    \item Nous observons également l'apparition de boucles dès que la victoire est détecté par une fonction d'évaluation. Cela est dû au fait que l'on cherche à trouver la victoire sans se soucier du chemin le plus cours pour l'atteindre. 
    En d'autres mots, nous détectons la victoire, mais n'y allons pas. Afin d'y remédier, nous avons essayer plusieurs approches, notamment un compteur vérifiant que la longueur du chemin vers la victoire doit diminuer à chaque itération ou en ajoutant un bonus au chemin les plus cours menant à la victoire dans la fonction d'évaluation ; malheureusement, ces méthodes se sont révélés inefficaces.
...
\begin{remark}
    L'évolution de la profondeur utilisé par les tests est la suivante : Moins de 15\% de graines récupéré au total implique une profondeur de 6, moins de 30\% une profondeur de 7, moins de 70\% une profondeur de 8, moins de 80\% une profondeur de 9, au-delà une profondeur de 10.
    Ces valeurs pourront évoluer lors de la compétition afin de limiter les instabilités dans le temps de calcul. 
\end{remark}

Maintenant que ces comparaisons ont été faites, il ne nous reste qu'une seule question relative à la valeur ajouté de la profondeur dynamique. 
...
Voici donc les résultats que nous avons eu avec \texttt{[J1]} en Alpha-Beta de profondeur 6 et \texttt{[J2]} en Alpha-Beta à profondeur adaptative :
\begin{table}[H]
    \centering
    \resizebox{\columnwidth}{!}{%
        \begin{tabular}{|r|ccc|c|}
...
Aussi, nous confirmons notre observations précédente que la fonction d'évaluation défensive du point de vue de \texttt{[J2]} se révèle être la plus efficace et celle qui profite le plus lorsqu'une différence de profondeur est observé entre les deux agents.
Cela revient à dire pour caricaturer qu'une bonne défense doit être préparé, tandis qu'une bonne attaque joue sur la vitesse en début de partie, qui semble au fur et à mesure des tests être le moment le plus important pour une stratégie offensive (la stratégie défensive a l'air moins dépendante de l'avancement de la partie).

Dans cette section, nous avons finalement démontré expérimentalement qu'une approche se basant sur une profondeur adaptative possède une réelle valeur ajoutée. De plus, une stratégie très offensive comme \textit{\textbf{corrected}} se révèle être le plus versatile et offre de meilleurs résultats dans un environment où l'adversaire possède une profondeur et une stratégie inconnue ce qui justifiera son utilisation pour \texttt{[J1]} et \texttt{[J2]}, même si la meilleur fonction d'évaluation pour \texttt{[J2]} semble être la stratégie défensive à grande différence de profondeur. 
