...
void play(struct GameState* game) {
    ...
    if (game->playJ1) {
        ...
        while (true) {
            ...
            // Ask move
            std::cout << "[J1] MinMax move advice: " << DecisionMinMax(game, findBestPmax(game)-1) << std::endl;
        ...
        while (true) {
            printBoard(game);
            std::cout << "[J2] AlphaBeta move advice: " << DecisionAlphaBeta(game, findBestPmax(game)) << std::endl;
